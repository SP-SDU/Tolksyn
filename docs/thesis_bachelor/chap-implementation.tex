...
The capture pipeline converts a captured label image into a stored extraction attempt. In \path{src/services/capture-pipeline.ts}, \path{processImage} performs this conversion in a fixed order: create an attempt id, persist the normalized image, create an attempt record, run barcode detection and remote extraction concurrently, optionally run manufacturer web-search enrichment, merge the evidence, and save the resulting extraction record.

The concurrency is deliberate. Barcode decoding and VLM extraction use the same persisted image but do not depend on each other, so \path{processImage} starts both with \path{Promise.all}. This reduces the time to the confirmation screen to approximately \path{max(barcode_time, extraction_time)} rather than \path{barcode_time + extraction_time}, assuming the device and network can run both tasks at the same time. The code also treats the two tasks differently when they fail. Barcode failure is logged and returns an empty list because missing barcode evidence should not block human review. Extraction failure becomes a fallback structured item with diagnostics because the confirmation screen still needs an attempt to display and a failure reason to explain.
...
On native platforms, \path{src/services/barcode-detector.ts} uses Expo Camera's \path{Camera.scanFromURLAsync} to scan the persisted image \cite{expo-camera}. On web, \path{src/services/barcode-detector.web.ts} first attempts the browser Barcode Detection API and falls back to the \path{barcode-detector} package when the native browser API is unavailable. The web implementation also limits image fetches with content-type, size, and timeout checks. MDN marks the Barcode Detection API as limited and experimental, so the fallback is needed for portability across browsers \cite{barcode-detection-api}.

The merge rule in \path{src/utils/merge-extraction-result.ts} is conservative. \path{mergeExtractionResult} deduplicates barcodes by type and data, stores the first decoded value as primary, and suggests \path{eanOrUpc} only for barcode types that are semantically appropriate for retail identifiers. If multiple EAN/UPC candidates exist, it records a conflict instead of silently choosing one. This is a direct human-in-the-loop tradeoff: preserve evidence and let the user resolve ambiguity instead of hiding uncertainty in an automatic field overwrite.

\section{Vision-Language Extraction}

Vision-language extraction is used because the problem is not only text recognition. Industrial labels combine text, layout, abbreviations, model numbers, manufacturer names, and sometimes country or compliance information. The model therefore has to map visible label evidence into a registration schema, not merely transcribe characters. The implementation uses a remote, schema-constrained extraction step rather than a local OCR pipeline \cite{ai-sdk}.
...
Settings also carry a security tradeoff. Provider credentials and ingest keys are required for BYOK operation, but they must not be mixed with ordinary JSON settings. \path{src/repositories/settings-repository.ts} stores non-secret configuration separately from provider auth and ingest API material. This aligns with the SecureStore design in the architecture \cite{expo-securestore}, while still acknowledging a platform limitation: web environments cannot provide the same native Keychain/Keystore guarantees as iOS and Android.

Manufacturer web-search enrichment is optional because it can conflict with label-derived evidence. In \path{src/services/manufacturer-websearch.ts}, the app first asks the extractor to plan at most three search queries, then uses a web-search dependency to query and fetch pages, then asks the extractor to reconcile those results with the original label extraction. The reconciliation prompt explicitly says that original label/image extraction and detected barcodes are primary evidence. Web evidence can fill null or ambiguous fields, but should not override label evidence without a conflict note.
...
\section{Dependency Observations}

The dependency stack is intentionally concentrated around Expo, React, React Native, TypeScript, Drizzle, Zod, AI SDK provider packages, Jest, and React Native Testing Library. These dependencies are not interchangeable labels. Each one is used to enforce a boundary in the implementation.

Expo provides the managed mobile runtime, camera, image picker, image manipulation, file storage, SQLite, and secure storage abstractions. Drizzle provides a typed SQL layer over Expo SQLite. Zod defines the runtime schema boundary, and the AI SDK provides the provider abstraction for image-capable language-model calls.

This dependency choice has practical limitations. It keeps the app inside Expo managed workflow and avoids custom native model runtimes, which supports portability across iOS, Android, and web. It also means true local VLM execution is not implemented and should not be implied by the architecture. A production handover should document the required Node version, lockfile policy, and Expo SDK version because React Native and Expo testing dependencies are sensitive to runtime versions.
