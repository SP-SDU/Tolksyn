...
export function ConfirmScreen({ attemptId }: { attemptId: string }) {
  ...
  const [attempt, setAttempt] =
    useState<Awaited<ReturnType<typeof runtime.attempts.getById>>>(null);
    ...
    useCallback(() => {
      ...
      Promise.all(
        CONFIRM_AUTOCOMPLETE_FIELDS.map(async (field) => {
          const distinct = await runtime.attempts.getDistinctFieldValues(
  ...
  const extractionAttempts =
    currentAttempt.extractionDiagnostics?.attempts ?? [];
  const detectedBarcodes =
    currentAttempt.extractionResult?.barcodeEnrichment.detected ?? [];
  const suggestedBarcode =
    currentAttempt.extractionResult?.barcodeEnrichment.relatedFieldSuggestions
      .eanOrUpc ?? "None";
  ...
  return (
    <View className="flex-1 bg-background">
          ...
          <BrutalFrame className="gap-2 bg-paper">
            <View className="flex-row flex-wrap gap-2">
              <StatusPill
                ...
                tone={
                  currentAttempt.status.endsWith("_failed")
                    ? "danger"
                    : "default"
                }
              />
              ...
              <StatusPill
                label={`Rev ${currentAttempt.acceptedRevision + 1}`}
                tone="warning"
              />
            </View>
            ...
            <FieldRow
              label="Created"
              value={new Date(currentAttempt.createdAt).toLocaleString()}
            />
          </BrutalFrame>
        </View>
        ...
        <BrutalFrame className="gap-4">
          ...
          {currentAttempt.extractionDiagnostics?.failed ? (
            <Text className="border-2 border-border bg-caution p-3 text-sm font-black uppercase tracking-wide text-foreground">
              VLM extraction failed. Fields are defaulted to null so you can
              edit and continue manually.
            </Text>
          ) : null}
          {editableFields.map(([key, value]) => {
            const isEnum =
              CONFIRM_ENUM_OPTIONS[key as keyof typeof CONFIRM_ENUM_OPTIONS] !=
              null;
          ...
          <BrutalFrame className="gap-3">
            ...
            {webSearch?.fieldChanges.length ? (
              webSearch.fieldChanges.map((change) => (
                ...
                >
                  <Text className="text-xs font-black uppercase tracking-wide text-foreground">
                    Changed: {formatLabel(change.field)}
                  </Text>
              ...
              <View className="gap-3">
                {extractionAttempts.map((item) => (
                  <View
                    ...
                    className="gap-1.5 border-2 border-border bg-panel px-3 py-2"
                  >
                    <Text className="text-xs font-black uppercase tracking-wide text-foreground">
                    ...
                    {item.error ? (
                      ...
                      <Text className="text-xs font-semibold text-signalBlue">
                        Success
                      </Text>
                    )}
                    <Text
      ...
      <OptionPicker
        ...
        selectedId={
          activePickerField && draft
            ? String(draft[activePickerField as keyof StructuredItem] ?? "")
            : undefined
        }
        items={
          activePickerField
            ? (CONFIRM_ENUM_OPTIONS[
                activePickerField as keyof typeof CONFIRM_ENUM_OPTIONS
              ] ?? [])
            : []
        }
