  ...
  return (
    <Screen className="gap-3">
      ...
      {attempts.map((attempt) => {
        const thumbnailUri = attempt.images[0]?.thumbnailUri ?? attempt.images[0]?.imageUri ?? null;
        ...
        return (
          <Pressable
            ...
            <View className="flex-row items-stretch gap-3 border-2 border-border bg-card p-3">
              {thumbnailUri ? (
                <ImagePreview uri={thumbnailUri} accessibilityLabel={`Thumbnail for attempt ${attempt.id}`} className="h-[78px] w-[78px] border-2 border-border bg-imageBase" />
              ) : (
                <View className="h-[78px] w-[78px] items-center justify-center border-2 border-border bg-imageBase">
                  <Text className="text-xs font-semibold text-muted">No image</Text>
                </View>
              )}
