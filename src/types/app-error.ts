...
export async function providerHttpStatusToError(response: Response): Promise<AppError> {
  const status = response.status;
  const detail = await providerErrorDetail(response);
  const message = detail.message ?? `Provider request failed with status ${status}.`;
  ...
  const ratelimitLike = normalized.includes('rate limit') || normalized.includes('too many requests');
...
async function providerErrorDetail(response: Response): Promise<{ normalized: string; message: string | null }> {
  try {
