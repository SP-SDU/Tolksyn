      ...
      try {
        ...
        if (isRetryableHttpStatus(response.status)) {
          return {
            ...
            errorCode: mapRetryableStatus(response.status),
          };
        }

        return {
          kind: "permanent_error",
          errorCode: mapPermanentStatus(response.status),
        };
      } catch (error) {
...
function mapPermanentStatus(
  status: number,
): "auth_failed" | "invalid_response" {
