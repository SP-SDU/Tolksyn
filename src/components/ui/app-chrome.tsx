...
export function BrutalFrame({ className, ...props }: ViewProps & { className?: string }) {
...
export function FieldRow({
  ...
  return (
    <View className="border-t-2 border-border py-3 first:border-t-0">
      <Text className="text-xs font-black uppercase tracking-wide text-muted">{label}</Text>
      <Text className={cn('mt-1 text-base font-semibold text-foreground', toneText[tone])}>{value}</Text>
    </View>
  );
}
...
export function StickyActionBar({ className, ...props }: ViewProps & { className?: string }) {
  return <View {...props} className={cn('border-t-2 border-border bg-background px-4 pb-5 pt-3', className)} />;
}
