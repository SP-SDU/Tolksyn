...
describe('web barcode detector', () => {
  ...
  test('uses native browser BarcodeDetector with normalized formats', async () => {
    const detect = jest.fn().mockResolvedValue([{ format: 'ean_13', rawValue: '4006381333931' }]);
    const BarcodeDetector = jest.fn().mockImplementation(() => ({ detect })) as jest.Mock & {
      getSupportedFormats: jest.Mock;
    };
    BarcodeDetector.getSupportedFormats = jest.fn().mockResolvedValue(['ean_13', 'code_93']);
    ...
    const results = await detector.detect({ imageUri: 'https://example.com/code.jpg', allowedTypes: ['ean13', 'code93', 'upc_e'] });

    expect(BarcodeDetector).toHaveBeenCalledWith({ formats: ['ean_13', 'code_93'] });
