import {
  ...
  toObjectValue,
} from "@/services/extraction/response-repair";
...
describe("remote extraction repair helpers", () => {
  ...
  test("parseLooseJson accepts raw, fenced, trailed-comma, and embedded JSON", () => {
    ...
    expect(
      parseLooseJson(
        'provider said {"note":"brace } in string","rows":[1,2,]} done',
      ),
    ).toEqual({
      ok: true,
    ...
    expect(
      parseLooseJson('prefix {"note":"\\\" } text","ok":true} suffix'),
    ).toEqual({
      ok: true,
  ...
  test("readEnvelopeValue accepts envelope aliases and safe fuzzy matches", () => {
    expect(
      readEnvelopeValue(
        { structuredJson: { sku: "camel" } },
        STRUCTURED_ENVELOPE_KEY,
      ),
    ).toEqual({ sku: "camel" });
    expect(
      readEnvelopeValue(
        { structured: { sku: "short" } },
        STRUCTURED_ENVELOPE_KEY,
      ),
    ).toEqual({ sku: "short" });
    expect(
      readEnvelopeValue(
        { strutured_json: { sku: "fuzzy" } },
        STRUCTURED_ENVELOPE_KEY,
      ),
    ).toEqual({ sku: "fuzzy" });
    expect(
      readEnvelopeValue({ auxiliary: "note" }, AUXILIARY_ENVELOPE_KEY),
    ).toBe("note");
    expect(
      readEnvelopeValue(
        { auxiliaryText: "alias text" },
        AUXILIARY_ENVELOPE_KEY,
      ),
    ).toBe("alias text");
    expect(
      readEnvelopeValue(
        { auxilary_text_optional: "fuzzy note" },
        AUXILIARY_ENVELOPE_KEY,
      ),
    ).toBe("fuzzy note");
    expect(
      readEnvelopeValue(
        { structured_jxxx: { sku: "limit" } },
        STRUCTURED_ENVELOPE_KEY,
      ),
    ).toEqual({ sku: "limit" });
    expect(
      readEnvelopeValue(
        {
          structuredJson: { sku: "first alias" },
          structured: { sku: "second alias" },
        },
        STRUCTURED_ENVELOPE_KEY,
      ),
  ...
  test("readEnvelopeValue rejects missing and ambiguous fuzzy envelope keys", () => {
    ...
    expect(
      readEnvelopeValue({ other: 1 }, STRUCTURED_ENVELOPE_KEY),
    ).toBeUndefined();
    expect(
      readEnvelopeValue(
        { structured_json_extra_far: 1 },
        STRUCTURED_ENVELOPE_KEY,
      ),
    ).toBeUndefined();
    expect(
  ...
  test("normalizeStructuredObjectKeys accepts fuzzy matches across distance limits", () => {
    ...
    expect(
      normalizeStructuredObjectKeys({ advancedInformotion: "details" }),
    ).toEqual({
  ...
  test("normalizeStructuredObjectKeys rejects unsafe fuzzy matches", () => {
    ...
    expect(
      normalizeStructuredObjectKeys({ productNumzzzz: "too far" }),
    ).toEqual({});
    expect(normalizeStructuredObjectKeys({ "!!!": "empty token" })).toEqual({});
    expect(
      normalizeStructuredObjectKeys({ completely_unrelated_key: "x" }),
    ).toEqual({});
  });
