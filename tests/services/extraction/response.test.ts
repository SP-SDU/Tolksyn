...
} from "@/services/extraction/errors";
...
describe("parseProviderJsonEnvelope", () => {
  ...
  test.each([
    ...
    "rejects invalid provider envelope with exact message %s",
    (input, message) => {
      const error = jest.spyOn(console, "error").mockImplementation(() => {});
...
describe("normalizeRemoteError", () => {
  ...
  ])("maps network failures %#", (error) => {
    expect(normalizeRemoteError(error)).toMatchObject({
      code: "network_unavailable",
      message: "The extraction request failed due to network unavailability.",
    });
  });
...
describe("remote extraction shared utilities", () => {
  ...
  test("formats provider error messages", () => {
    ...
    expect(providerErrorMessage(new Error("plain failed"))).toBe(
      "plain failed",
    );
