...
class MemoryQueueRepository implements QueueRepository {
  ...
  async peekReady(now: number): Promise<QueueItem | null> {
    return this.items.find((item) => item.nextAttemptAt <= now && item.status === 'queued') ?? null;
  }
  ...
  async reschedule(id: string, nextAttemptAt: number, retryCount: number, errorCode: string): Promise<void> {
    const item = this.items.find((entry) => entry.id === id);
    if (item) {
...
describe('drainQueue', () => {
  test('drains ready items in FIFO order', async () => {
    ...
    expect(repository.snapshot().map((item) => item.status)).toEqual(['sent', 'sent']);
  });
  ...
  test('marks permanent failures without retrying later items ahead of them', async () => {
    ...
    expect(repository.snapshot()).toEqual([
      expect.objectContaining({ id: '1', status: 'failed', lastErrorCode: 'auth_failed' }),
